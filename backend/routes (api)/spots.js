...
const queryCreateSpot = function (
  db,
  userID,
  spot_text,
  is_respot,
  ...
  RETURNING *`;
  return db
    .query(query, [userID, spot_text, is_respot, spotify_json])
    .then((data) => {
...
const queryAllSpotsFromFollowing = function (db, userID) {
  ...
  return db.query(query, [userID]).then((data) => {
    return data.rows;
  });
};
...
module.exports = (db) => {
  ...
  router.post("/create", (req, res) => {
    queryCreateSpot(
      db,
      1,
      req.body.spotTextValue,
      false,
      req.body.selectedSongData
    )
      .then((data) => {
        res.json(data);
      })
  ...
  router.get("/:id/following", (req, res) => {
    queryGetUserById(db, req.params.id).then((user) => {
